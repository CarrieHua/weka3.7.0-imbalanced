...
public class AdaBoostM2 
  ...
  protected void buildClassifierWithWeights(Instances data) 
    ...
    // with it doesn't mess up the weights for anyone else
    training = new Instances(data, 0, numInstances);
    sumWeights = training.sumOfWeights();
    for (int i = 0; i < training.numInstances(); i++) {
      training.instance(i).setWeight(training.instance(i).
				      weight() / sumWeights);
    }
   
    double[] D=new double[numInstances];
    double[] W=new double[numInstances];
    double[][] w=new double[numInstances][m_NumClasses-1];
    double[][] q=new double[numInstances][m_NumClasses-1];
    //Initialize D and w
    for(int i=0;i<D.length;i++){
    	D[i]=training.instance(i).weight();
    	for(int j=0;j<w[i].length;j++){
    		w[i][j]=D[i]/(m_NumClasses-1);
    	}
    }
	...
	((Randomizable) m_Classifiers[m_NumIterationsPerformed]).setSeed(randomInstance.nextInt());
     ...
     for(int i=0;i<D.length;i++){
    	 D[i]=W[i]/(Utils.sum(W));
    	 trainData.instance(i).setWeight(D[i]);
     }
      ...
      for(int i=0;i<numInstances;i++){
    	  Instance ins=training.instance(i);
    	  int classLabel=(int)ins.classValue();
    	  double[] prob=m_Classifiers[m_NumIterationsPerformed].distributionForInstance(ins); 
    	  
    	  double[] tempProb=new double[m_NumClasses-1];
    	  for(int j=0,k=0;j<prob.length;){
    		  if(j!=classLabel){
    			  tempProb[k]=prob[j];
    			  k++;
    			  j++;
    	  ...
    	  double[] tempSum=new double[m_NumClasses-1];
    	  for(int j=0;j<tempSum.length;j++){
    		  tempSum[j]=q[i][j]*tempProb[j];
    	  }
    	  epsilon=epsilon+(D[i]*(1-prob[classLabel]+Utils.sum(tempSum)))/2;
      } 
      ...
      for(int i=0;i<numInstances;i++){
    	  ...
    	  double[] prob=m_Classifiers[m_NumIterationsPerformed].distributionForInstance(ins); 
    	  for(int j=0;j<w[i].length;j++){
    		w[i][j]=w[i][j]*Math.pow(epsilon/(1-epsilon), (1+prob[classLabel]-prob[j])/2);
    	  }
      }
  ...
  public double [] distributionForInstance(Instance instance) 
    ...
    if (m_NumIterationsPerformed == 1) {
      ...
      for (int i = 0; i < m_NumIterationsPerformed; i++) {
    	  double[] probs=m_Classifiers[i].distributionForInstance(instance);
    	  for(int j=0;j<probs.length;j++){
    		  sums[j]=sums[j]+m_Betas[i]*probs[j];
    	  }
  ...
  public static void main(String [] argv) {
    runClassifier(new AdaBoostM2(), argv);
  }
}
