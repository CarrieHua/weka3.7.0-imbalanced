...
public class SplitVote
  ...
  public Instances createSampleData(Instances data, int i){
	  ...
	  //少数类实例
	  for(int j=0;j<classNum[1-majorClass];j++){
		  sample.add(data.instance(j+startNum));
	  }
	  
	  //多数类实例
	  int num=classNum[majorClass] / m_Classifiers.length;
  ...
  public static void main(String [] argv) {
    runClassifier(new SplitVote(), argv);
  }
}
