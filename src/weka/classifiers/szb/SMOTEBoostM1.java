 ...
 */
public class SMOTEBoostM1 extends RandomizableIteratedSingleClassifierEnhancer
	...
	protected void buildClassifierUsingResampling(Instances data)
		...
		Instances trainData, sample, training,sampleTraining;
		...
		for (m_NumIterationsPerformed = 0; m_NumIterationsPerformed < m_Classifiers.length; m_NumIterationsPerformed++) {
			...
			do{
				sample = trainData.stratifiedResampleWithWeights(randomInstance, weights);
				//smote
				MySMOTE mysmote = new MySMOTE();
				mysmote.setInputFormat(sample);
				sampleTraining = Filter.useFilter(sample, mysmote);
				sampleTraining.randomize(randomInstance);
				
				// Build and evaluate classifier
				m_Classifiers[m_NumIterationsPerformed].buildClassifier(sampleTraining);
	...
	protected void setWeights(Instances training, double reweight)
		...
		while (enu.hasMoreElements()) {
			...
			double[] results=m_Classifiers[m_NumIterationsPerformed].distributionForInstance(instance);
			
			double temp=1+results[(int)classValue];
			...
			temp=0.5*temp;
			instance.setWeight(instance.weight() * Math.pow(reweight,temp));
		}
	...
	public double[] distributionForInstance(Instance instance) throws Exception {
		...
		if (m_NumIterationsPerformed == 1) {
			...
			for (int i = 0; i < m_NumIterationsPerformed; i++) {
				double[] results=m_Classifiers[i].distributionForInstance(instance);
				for(int j=0;j<instance.numClasses();j++){
					sums[j]+=m_Betas[i]*results[j];
				}
			}
	...
	public String toString() {
		...
		if (m_NumIterationsPerformed == 0) {
			...
			text
					.append("AdaBoostM1: No boosting possible, one classifier used!\n");
	...
	public static void main(String[] argv) {
		runClassifier(new SMOTEBoostM1(), argv);
	}
}
