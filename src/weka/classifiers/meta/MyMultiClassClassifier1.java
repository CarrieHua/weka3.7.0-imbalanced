...
import szb.GaussianElimination;
 ...
 */
public class MyMultiClassClassifier1 extends
		RandomizableSingleClassifierEnhancer implements OptionHandler {
	 ...
	 * Constructor.
	 */
	public MyMultiClassClassifier1() {
	...
	private abstract class Code implements Serializable, RevisionHandler {
		...
		/**
		 * Subclasses must allocate and fill these. First dimension is number of
		 * codes. Second dimension is number of classes.
		 */
		...
		/**
		 * Returns the indices of the values set to true for this code, using
		 * 1-based indexing (for input to Range).
		 * 
		 * @param which
		 *            the index
	...
	/*
	 * TODO: Constructs codes as per: Bose, R.C., Ray Chaudhuri (1960), On a
	 * class of error-correcting binary group codes, Information and Control, 3,
	 * 68-79. Hocquenghem, A. (1959) Codes corecteurs d'erreurs, Chiffres, 2,
	 * 147-156.
	 */
	...
	public void buildClassifier(Instances insts) throws Exception {
		...
		if (numClassifiers <= 2) {
			...
			m_TwoClassDataset.insertAttributeAt(new Attribute("class",
			...
			case METHOD_ERROR_RANDOM:
				code = new RandomCode(numClassifiers,
						(int) (numClassifiers * m_RandomWidthFactor), insts);
				break;
	...
	/**
	 * Returns the individual predictions of the base classifiers for an
	 * instance. Used by StackedMultiClassClassifier. Returns the probability
	 * for the second "class" predicted by each base classifier.
	 * 
	 * @param inst
			...
			for (int i = 0; i < m_ClassFilters.length; i++) {
				if (m_Classifiers[i] != null) {
					if (m_Method == METHOD_1_AGAINST_1) {
						...
						result[i] = m_Classifiers[i]
								.distributionForInstance(m_ClassFilters[i]
										.output())[1];
					}
				}
			}
	...
	public double[] distributionForInstance(Instance inst) throws Exception {
		...
		//search the k neighbours
		 IBk knn =new IBk();
		 knn.setKNN(5);
		 try{
			 knn.buildClassifier(m_Train);
			 Instances neighbours = new Instances(knn
						.ReturnNeighbourForInstance(inst));  
			 if(m_Method== METHOD_1_AGAINST_1){
				 for(int i=0;i<neighbours.numInstances();i++)
				 {
					 double[][] r = new double[inst.numClasses()][inst.numClasses()];
				     double[][] n = new double[inst.numClasses()][inst.numClasses()]; 
				     Instance copy= neighbours.instance(i);
				     for(int j=0;j<m_ClassFilters.length;j++){
				    	 if(m_ClassFilters[j]!= null){
				    		Instance tempInst = (Instance)copy.copy(); 
				    		tempInst.setClassMissing();
				    		tempInst.setDataset(m_TwoClassDataset);
				    		double [] current = m_Classifiers[j].distributionForInstance(tempInst);  
				    		Range range = new Range(((RemoveWithValues)m_ClassFilters[j])
				    					  .getNominalIndices());
				    		range.setUpper(m_ClassAttribute.numValues());
				    		int[] pair = range.getSelection();
				    		if (m_pairwiseCoupling && inst.numClasses() > 2) {
				                r[pair[0]][pair[1]] = current[0];
				                n[pair[0]][pair[1]] = m_SumOfWeights[j];
				              }
				    	 }
				     }
				     if (m_pairwiseCoupling && inst.numClasses() > 2) {
				          double [] tempProbs=pairwiseCoupling(n,r);
				          probs[(int) copy.classValue()]+=tempProbs[(int) copy.classValue()];
				     }
				 }
			 }
		 }catch(Exception e){
				...
				if ((m_ClassFilters != null) && (m_ClassFilters[i] != null)) {
					if (m_ClassFilters[i] instanceof RemoveWithValues) {
						Range range = new Range(
								((RemoveWithValues) m_ClassFilters[i])
										.getNominalIndices());
						range.setUpper(m_ClassAttribute.numValues());
						int[] pair = range.getSelection();
						text.append(", " + (pair[0] + 1) + " vs "
								+ (pair[1] + 1));
					} else if (m_ClassFilters[i] instanceof MakeIndicator) {
						text.append(", using indicator values: ");
						text.append(((MakeIndicator) m_ClassFilters[i])
								.getValueRange());
					}
				}
				text.append('\n');
	...
	public String[] getOptions() {
		...
		System
				.arraycopy(superOptions, 0, options, current,
						superOptions.length);
	...
	/**
	 * Set whether to use pairwise coupling with 1-vs-1 classification to
	...
	/**
	 * Gets whether to use pairwise coupling with 1-vs-1 classification to
	 * improve probability estimates.
	 * 
	...
	public static double[] pairwiseCoupling1(double[][] n, double[][] r) {
		...
		double[] p = new double[r.length];
		GaussianElimination ge = new GaussianElimination(r.length);

		// 根据概率估计数组r求矩阵Q
		double[][] Q = new double[r.length][r.length];
		for (int i = 0; i < r.length; i++) {
			for (int j = 0; j < r.length; j++) {
				if (i == j) {
					...
					Q[i][j] = sum / (r.length - 1);
				} else {
					Q[i][j] = r[i][j] / (r.length - 1);
				}
			}
		}
	...
	public static double[] pairwiseCoupling2(double[][] n, double[][] r) {
		double[] p = new double[r.length];
		// 概率估计数组进行转化
		for (int i = 0; i < r.length - 1; i++) {
			for (int j = i + 1; j < r.length; j++) {
		...
		GaussianElimination ge = new GaussianElimination(r.length + 1);
	...
	public static double[] CalculateDistance(Instances data, Instance instance,
			int k) {

		double[] p = new double[data.numClasses()];
		int[] num = new int[data.numClasses()];
		...
		try {
			...
			Instances neighbours = new Instances(knn
					.ReturnNeighbourForInstance(instance));
			for (int i = 0; i < neighbours.numInstances(); i++) {
				Instance inst = neighbours.instance(i);
				for (int j = 0; j < neighbours.numClasses(); j++) {
					if (inst.classValue() == j) {
						num[j] = num[j] + 1;
					}
				}
			}
			...
			for (int i = 0; i < data.numClasses(); i++) {
				p[i] = num[i] / (double) neighbours.numInstances();
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void main(String[] argv) {
		runClassifier(new MyMultiClassClassifier1(), argv);
	}
}
