...
public class UnderBagging
  ...
  public static final int CONFIDENCE_VOTE=11;
  ...
  public static final Tag[] TAGS_RULES = {
    ...
    new Tag(CONFIDENCE_VOTE,"CFD","Confidence voting rule")
  };
  ...
  public void buildClassifier(Instances data) throws Exception {
    ...
    if (m_attribSelection){
    	for (int i=0; i< m_Classifiers.length;i++){
			...
			FASTSearch search = new FASTSearch();
    ...
    for (int i = 0; i < m_Classifiers.length; i++) {   
      Instances sampleData = randomUnderSampling(newData, majC, 50);
   ...
   * @return
   */
  protected Instances randomUnderSampling(Instances copia, int majC, int N)
  {
    ...
    int size = copia.attributeStats(copia.classIndex()).nominalCounts[majC == 0 ? 1 : 0] * (100 + 2 * N) / 100;
    ...
    for (int i = 0; i < copia.numInstances(); i++){
        ...
        {
        	//selected[nData] = i;
        	myDataset.add(copia.instance(i));
        }
     }
  ...
  public static void main(String [] argv) {
    runClassifier(new UnderBagging(), argv);
  }
}
