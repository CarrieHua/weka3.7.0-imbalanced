 ...
 */
public class FSforClu2 extends Filter implements SupervisedFilter, OptionHandler {
...
//	protected void convertInstanceFSforClu(Instance instance,int[] attsTodel,int numTodel) throws Exception {
//	    double[] newVals = new double[getOutputFormat().numAttributes()];
//
//	    
//	    for (int i = 0; i < clusterNum+1; i++) {
...
//					father[jj] = jj;
	...
	public boolean batchFinished() throws Exception {
		...
		if (!isOutputFormatDefined()) {
			...
			System.out.print("data\t\t"+feature+"\t"+data.numInstances()+"\n");
			
			
			
			Instances disData,disData1,data1,data1sample,disData1sample;
			
			Filter m_Filter = new weka.filters.supervised.attribute.Discretize();
			m_Filter.setInputFormat(data); 	
			disData = Filter.useFilter(data, m_Filter);  //discretize			
						
			
			Filter m_Filter2 = new weka.filters.unsupervised.attribute.RemoveFSforClu();		
			m_Filter2.setInputFormat(disData); 
			disData1 = Filter.useFilter(disData, m_Filter2);  //removeFSforClu;						
			System.out.print("disData1\t"+disData1.numAttributes()+"\t");
			System.out.print(disData1.numInstances()+"\n");
			
			
			
			//----------------------------------
			
			int [] attstodel = new int[data.numAttributes()];
			int[] numTodel = new int [1];
		
			m_Filter2.attstodel(attstodel,numTodel);
			int numdel= numTodel[0];
			
			//------------------------------
			Filter m_Filter3 = new weka.filters.unsupervised.attribute.RemoveFSforClu();			
			m_Filter3.setInputFormat(data); 
			data1 = Filter.useFilterFSforClu(data, m_Filter3, attstodel, numdel  );  
			//remove Features in the number data;
			System.out.print("data1\t\t"+data1.numAttributes()+"\t");
			System.out.print(data1.numInstances()+"\n");

		

			//**************sample
			Filter m_Filter4 = new weka.filters.unsupervised.attribute.RemoveFSforClu();
			m_Filter4.setInputFormat(data1); 
			data1sample = Filter.usesamplenjj(data1, m_Filter4);  
			//sample;
			
			System.out.print("datasamp\t"+data1sample.numAttributes()+"\t");
			System.out.print(data1sample.numInstances()+"\n");
			
			
//			data1sample=data1;
			//****************clustering for class 

			int claindex=data1sample.classIndex();
//			
//			for(int i=0;i<data1sample.numInstances();i++){
//				System.out.print(data1sample.instance(i).value(claindex)+"\t");				
//			}
//			System.out.print("\n");
			
			data1sample.setClassIndex(-1);
			data1sample.deleteAttributeAt(claindex);
				
			EM em = new EM();
			em.buildClusterer(data1sample);
			int clusternum= em.numberOfClusters();
			int [] id=new int[data1sample.numInstances()];
			
			for(int i=0;i<data1sample.numInstances();i++){
				id[i] = em.clusterInstance(data1sample.instance(i));
//				System.out.print(id[i]+"\t");
			}
			System.out.print("\n");
			
			
			String attributeName = "cluclassindex";
			FastVector attributeValues = new FastVector(clusternum);
			
			for(int i=0;i<clusternum;i++){
				 attributeValues.addElement(""+i);
			}
				
			Attribute att= new Attribute(attributeName,attributeValues);
	
			data1sample.insertAttributeAt(att, data1sample.numAttributes());
			
			
			int numA = data1sample.numAttributes()-1;
			data1sample.setClassIndex(numA);
			
			for(int i=0;i<data1sample.numInstances();i++){
			
				data1sample.instance(i).setValue(numA, id[i]);		
			}
			System.out.print("\n");
...
//	
			...
			//****FAST***//
//			njjData	= data1sample;
			Filter m_Filter5 = new weka.filters.supervised.attribute.Discretize();
			m_Filter.setInputFormat(data1sample); 	
			njjData = Filter.useFilter(data1sample, m_Filter);  //discretize			
			System.out.print(njjData.numAttributes()+"\t"+njjData.numInstances()+"\n");
			
			Filter m_Filter1 = new weka.filters.unsupervised.attribute.RemoveUseless();
			m_Filter1.setInputFormat(njjData); 
			njjData = Filter.useFilter(njjData, m_Filter1);  //removeUseless;
			System.out.print(njjData.numAttributes()+"\t"+njjData.numInstances()+"\n");
...
//			AppendToFile.appendMethodA("result/NJJ/Time.txt", ","+clusterNum+","+data.numAttributes()+","+njjData.numAttributes()+",");
			...
			m_OutputFormat.setRelationName(clusterNum+",-1-,"+ data.numAttributes()+ "," +name  );
//			 Convert pending input instances
			for (int i = 0; i <njjData.numInstances(); i++) {
				convertInstancenjj(njjData.instance(i), GG, clusterNum);
			}
			AppendToFile.appendMethodA("D:/exp/result/Time1.txt", ","+clusterNum+",");
	...
	private void runClusterer(EM em, String[] argv) {
...
//		setSearch(new BestFirst());
	...
	public static void main(String[] argv) {
		File file = new File("D:/exp/67dataset/");
		AppendToFile.appendMethodB("D:/exp/result/Time1.txt", " ");
...
//					"-S", "weka.attributeSelection.BestFirst -S 8",
					...
					"-i", "D:/exp/67dataset/" + lf[i].getName(), 
					"-o", "D:/exp/result/" + lf[i].getName()
			};
			AppendToFile.appendMethodA("D:/exp/result/Time1.txt", lf[i].getName());
			long st =System.currentTimeMillis();
			runFilter(new FSforClu2(), arg);
			long end =System.currentTimeMillis();
			long time=end-st;
			AppendToFile.appendMethodA("D:/exp/result/Time1.txt", time + "\r\n");
		}
	}
