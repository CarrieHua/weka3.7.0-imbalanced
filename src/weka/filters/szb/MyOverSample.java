 ...
 * The original dataset must fit entirely in memory. The number of instances in the generated dataset may be specified. The dataset must have a nominal class attribute. If not, use the unsupervised version. The filter can be made to maintain the class distribution in the subsample, or to bias the class distribution toward a uniform distribution. When used in batch mode (i.e. in the FilteredClassifier), subsequent batches are NOT OverSampled.
 * <p/>
 <!-- globalinfo-end -->
 * 
 <!-- options-start -->
...
public class MyOverSample
  ...
  public String globalInfo() {
    return 
      ...
      + "mode (i.e. in the FilteredClassifier), subsequent batches are NOT OverSampled.";
  }
  ...
  protected void createSubsample() {
	...
	//automatically detect the minority class
	int majIndex=0;
	int minIndex=0;
	int maj=0;
	int min=Integer.MAX_VALUE;
	int classNum[]=getInputFormat().attributeStats(getInputFormat().classIndex()).nominalCounts;
	for(int i=0;i<classNum.length;i++){
	   if(classNum[i]!=0 && classNum[i]<min){
	    	min=classNum[i];
	    	minIndex=i;
	    }
	    if(classNum[i]>maj){
    ...
    //sample
    int sampleSize = (int) ((maj) * m_SampleSizePercent / 100) - min;
    if (sampleSize > 0){
    	Random random = new Random(m_RandomSeed);
    	for (int j = 0; j < sampleSize; j++){
    		int i = random.nextInt(min);
  ...
  public static void main(String [] argv) {
    runFilter(new MyOverSample(), argv);
  }
}
