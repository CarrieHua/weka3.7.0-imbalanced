 ...
 * <p/>
 * The difference from the origin SMOTE in weka is that you can specify the ratio between the 
 * minority and majority (which is 1:2 default) instead of specifying the amount of SMOTE directly. 
 <!-- globalinfo-end -->
 *
...
public class MySMOTE
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (percentageStr.length() != 0) {
      setRatio(new Double(percentageStr).doubleValue());
    } else {
      setRatio(0.5);
    }
  ...
  /**
   * Sets the ratio of SMOTE instances to create.
   * 
   * @param value	the percentage to use
   */
  ...
  /**
   * Gets the ratio of SMOTE instances to create.
   * 
   * @return 		the percentage of SMOTE instances to create
   */
  ...
  protected void doSMOTE() throws Exception {
    int minIndex = 0;
    int majIndex = 0;  // the index of the major class label
    int maj = 0;
    ...
    /////calculate the percentage by the ratio m_Ratio
    if (maj > 0){
    	m_Percentage = (m_Ratio * maj / min - 1) * 100;
//    	System.out.println(m_Percentage);
    	if (m_Percentage <= 0){
    		m_Percentage = 0;
    		return;
    	}
	...
	public int compare(Object o1, Object o2) {
	  ...
	  return  Double.compare(distance1, distance2);
			  //(int) Math.ceil(distance1 - distance2);
	}
      });
