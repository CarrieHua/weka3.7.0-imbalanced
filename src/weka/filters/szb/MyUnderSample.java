 ...
 * The original dataset must fit entirely in memory. The number of instances in the generated dataset may be specified. The dataset must have a nominal class attribute. If not, use the unsupervised version. The filter can be made to maintain the class distribution in the subsample, or to bias the class distribution toward a uniform distribution. When used in batch mode (i.e. in the FilteredClassifier), subsequent batches are NOT UnderSampled.
 * <p/>
 <!-- globalinfo-end -->
 * 
 <!-- options-start -->
...
public class MyUnderSample
  ...
  protected void createSubsample() {
   ...
   //automatically detect the minority class
    ...
    //sample
    int sampleSize = (int) (min * 100 / m_SampleSizePercent);
    if (sampleSize > 0){
    	Random random = new Random(m_RandomSeed);
    	for (int j = 0; j < sampleSize; j++){
    		int i = random.nextInt(maj);
  ...
  public static void main(String [] argv) {
    runFilter(new MyUnderSample(), argv);
  }
}
