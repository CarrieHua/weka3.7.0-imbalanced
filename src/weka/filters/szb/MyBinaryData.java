...
/** 
 <!-- globalinfo-start -->
 * 将多类数据集转化为人工构造的二类不均衡数据集
 *参考文献:{Evolutionary Undersampling for Classification with Imbalanced Datasets: Proposals and Taxonomy}
 * <p/>
 <!-- globalinfo-end -->
 * 
 <!-- options-start -->
...
public class MyBinaryData 
  ...
  public boolean batchFinished() {
    ...
    Instances OrigData = getInputFormat();
    OrigData.randomize(m_Random);
    OrigData.deleteWithMissingClass();
    Instances DataCopy=new Instances(OrigData);
    Instances OutputData=new Instances(DataCopy,0);
    if (!isFirstBatchDone()) 
      {                  
            ...
            OutputData.setClassIndex(0);
            OutputData.deleteAttributeAt(OutputData.numAttributes()-1);
            OutputData.insertAttributeAt(new Attribute("Class", ClassValues), OutputData.numAttributes());
            OutputData.setClassIndex(OutputData.numAttributes()-1);
            setOutputFormat(OutputData);
           /* 
            for(int i=0;i<DataCopy.numInstances();i++)
            {
            	Instance ins=DataCopy.instance(i);
            	if(ins.classValue()!=8&&ins.classValue()!=0)
            	{
            		DataCopy.delete(i);
            		i--;
            	}
            ...
            */
            for(int i=0;i<DataCopy.numInstances();i++)
            {
            	...
            	{
            		OutputData.add(ins);
            		OutputData.instance(i).setClassValue(OutputData.classAttribute().value(0));
            	}
            	else
            	{
            		OutputData.add(ins);
            		OutputData.instance(i).setClassValue(OutputData.classAttribute().value(1));
            	}
            }
            
            for (int i = 0; i < OutputData.numInstances(); i++) 
            {
              push(OutputData.instance(i));
            }
  ...
  public static void main(String [] argv) {
    runFilter(new MyBinaryData(), argv);
  }
}
