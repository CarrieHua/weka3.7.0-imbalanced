...
public abstract class Filter
  ...
  public static Instances usesamplenjj(Instances data,
		    Filter filter) throws Exception {
/*
System.err.println(filter.getClass().getName() 
             + " in:" + data.numInstances());
...
return newData;
  ...
  public static Instances useFilterFSforClu(Instances data,
		    Filter filter,int [] attsTodel, int numdel) throws Exception {
/*
System.err.println(filter.getClass().getName() 
             + " in:" + data.numInstances());
*/
...
filter.batchFinishedFSforClu(attsTodel, numdel);
Instances newData = filter.getOutputFormat();
Instance processed;
while ((processed = filter.output()) != null) {
newData.add(processed);
...
return newData;
  ...
  public static Instances useFilternjj(Instances data,
		    Filter filter) throws Exception {
/*
...
return newData;
  ...
  public static void njjfilterFile(Filter filter, String [] options) 
  throws Exception {
  ...
  protected static void njjrunFilter(Filter filter, String[] options) {
	    try {
	      if (Utils.getFlag('b', options)) {
		Filter.batchFilterFile(filter, options);
	      } else {
		Filter.njjfilterFile(filter, options);
	      }
	    } catch (Exception e) {
	      if (    (e.toString().indexOf("Help requested") == -1) 
		   && (e.toString().indexOf("Filter options") == -1) )
