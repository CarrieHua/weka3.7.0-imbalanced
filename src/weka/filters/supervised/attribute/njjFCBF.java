 ...
 */
public class njjFCBF extends Filter implements SupervisedFilter, OptionHandler {
...
//	public void BFS2(int i, njjGraph1 tree, double[] suic, int[] ranked) {
//		for (int j = 0; j < tree.adjList.elementAt(i).size(); j++) { // 3
//			int jj = tree.adjList.elementAt(i).elementAt(j).v2;
//			if (!visited[jj]) { // 4
//				visited[jj] = true;
//				if ((suij[i][jj] < (1-suic[ranked[i]]))&&(suij[i][jj] <(1- suic[ranked[jj]]))) {
//					GG[clusterNum] = ranked[jj];
//					clu[jj] = clusterNum;
//					clusterNum++;
////					father[jj] = jj;
...
//					}
//				} // 5/
//				BFS2(jj, tree, suic, ranked);
//			}
//		}// 3/
//	}
	
	public void BFS(int i, njjGraph1 tree, double t, int[] ranked) {
		for (int j = 0; j < tree.adjList.elementAt(i).size(); j++) { // 3
			int jj = tree.adjList.elementAt(i).elementAt(j).v2;
			if (!visited[jj]) { // 4
				visited[jj] = true;
//				System.out.print(suij[i][jj] +"\t"+(1-suic[ranked[i]])+"\t"+(1- suic[ranked[jj]])+"\n");
				if ((suij[i][jj] < (1-suic[ranked[i]]))||(suij[i][jj] <(1- suic[ranked[jj]]))) {
					GG[clusterNum] = ranked[jj];
					clu[jj] = clusterNum;
					clusterNum++;
...
//			njjGraph1 tree = njjg.njjprimsu5(suij, 0,a,suijave);	
			njjGraph1 tree = njjg.njjprimsuFCBF(suij, 0);	
...
//			AppendToFile.appendMethodA("result/NJJ/Time.txt", ","+clusterNum+","+data.numAttributes()+","+njjData.numAttributes()+",");
			...
			m_OutputFormat.setRelationName(clusterNum+",-9-,"+ data.numAttributes()+ ","+name );
			// Convert pending input instances
			for (int i = 0; i <njjData.numInstances(); i++) {
				convertInstancenjj(njjData.instance(i), GG, clusterNum);
			}
			
			AppendToFile.appendMethodA("D:/exp/njj9/Time9.txt", ","+clusterNum+",");
	...
	public static void main(String[] argv) {
		File file = new File("D:/exp/dataset/");
		AppendToFile.appendMethodB("D:/exp/njj9/Time9.txt", " ");
		...
		for (int i = 0; i < lf.length; i++) {
			...
			String[] arg = {
					...
					"-i", "D:/exp/dataset/" + lf[i].getName(), 
					"-o", "D:/exp/njj9/" + lf[i].getName()
			};
			long st =System.currentTimeMillis();
			runFilter(new njjFCBF(), arg);
			long end =System.currentTimeMillis();
			long time=end-st;
			AppendToFile.appendMethodA("D:/exp/njj9/Time9.txt", lf[i].getName()+","+time + ",\r\n");
		}
		}
	}
}
