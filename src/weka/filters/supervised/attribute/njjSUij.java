 ...
 */
public class njjSUij extends Filter implements SupervisedFilter, OptionHandler {
...
//			for(int i=0;i<yuzhi;i++)
//			System.out.print(clu[i]+"\t");
			
			int[][] index =new int[clusterNum][yuzhi];
			double[][] SUicc = new double[clusterNum][yuzhi];
			int[] tag =new int[clusterNum];
			
			for(int i=0;i<yuzhi;i++){
				int c = clu[i];
				double su = suic[ranked[i]];
				index[c][tag[c]]= i;			
				SUicc[c][tag[c]]=su;
				tag[c]++;
			}
			...
			for(int i=0;i<clusterNum-1;i++){
				double su[]=new double[tag[i]];
				for(int j=0;j<tag[i];j++){
					su[j]=SUicc[i][j];
				}
				int[] tem = Utils.sort(su);
				int t=tag[i]/2;
				int mid = tem[t];
				GG[i]= index[i][mid];
			}
...
//			AppendToFile.appendMethodA("result/NJJ/Time.txt", ","+clusterNum+","+data.numAttributes()+","+njjData.numAttributes()+",");
			...
			m_OutputFormat.setRelationName(clusterNum+",-8-,"+ data.numAttributes()+ "," +name  );
			// Convert pending input instances
			for (int i = 0; i <njjData.numInstances(); i++) {
				convertInstancenjj(njjData.instance(i), GG, clusterNum);
			}
			AppendToFile.appendMethodA("D:/exp/njj8/Time8.txt", ","+clusterNum+",");
	...
	public static void main(String[] argv) {
		...
		AppendToFile.appendMethodB("D:/exp/njj8/Time8.txt", " ");
		...
		for (int i = 0; i < lf.length; i++) {
			...
			String[] arg = {
					...
					"-o", "D:/exp/njj8/" + lf[i].getName()
			};
			AppendToFile.appendMethodA("D:/exp/njj8/Time8.txt", lf[i].getName());
			long st =System.currentTimeMillis();
			runFilter(new njjSUij(), arg);
			long end =System.currentTimeMillis();
			long time=end-st;
			AppendToFile.appendMethodA("D:/exp/njj8/Time8.txt", time + "\r\n");
		}
	}
}
