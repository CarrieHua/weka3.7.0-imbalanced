 ...
 */
public class FASTminDraw extends Filter implements SupervisedFilter, OptionHandler {
...
//	}
	...
	public void BFS(int i, njjGraph1 tree, int[] ranked) {
		for (int j = 0; j < tree.adjList.elementAt(i).size(); j++) { // 3
			int jj = tree.adjList.elementAt(i).elementAt(j).v2;
			if (!visited[jj]) { // 4
				visited[jj] = true;
				if (suij[i][jj] < (1-suic[ranked[jj]]) && suij[i][jj] < (1-suic[ranked[i]])) {
...
//			Filter m_Filter = new weka.filters.supervised.attribute.njjDiscretizeTrue();
			...
			int yuzhi;
//			double yuzhi1,yuzhi2,yuzhi3;
//			yuzhi1=Math.log(njjFeatureNum);
//			yuzhi2=Math.log(10);
//			yuzhi3=Math.pow(njjFeatureNum, 0.5);
//			yuzhi=(int)(yuzhi3*yuzhi1/yuzhi2);  //filter yuzhi=N/logN base 2
			
//			yuzhi=K;
			
			yuzhi=(int)(Y*(double)njjFeatureNum);
...
//			System.out.print(suicave+"\t");		
			...
			njjGraph1 tree = njjg.njjprimsuMIN(suij, 0,a);	
...
//			AppendToFile.appendMethodA("result/NJJ/Time.txt", ","+clusterNum+","+data.numAttributes()+","+njjData.numAttributes()+",");
			...
			m_OutputFormat.setRelationName(Y+","+ data.numAttributes()+ "," +njjData.numAttributes()+","+yuzhi+","+clusterNum);
			// Convert pending input instances
			for (int i = 0; i <njjData.numInstances(); i++) {
				convertInstancenjj(njjData.instance(i), GG, clusterNum);
			}
			AppendToFile.appendMethodA("D:/FASTminDraw/"+y+"/T.txt", ","+data.numAttributes()+ ","+Y+"," +njjData.numAttributes()+","+yuzhi+","+clusterNum+",");
...
//		setSearch(new BestFirst());
	...
	public static void main(String[] argv) {
		...
		double yuzhi=0.41125;
		String y = String.valueOf(yuzhi);
		
		
		
		AppendToFile.appendMethodA("D:/FASTminDraw/"+y+"/T.txt", "\r\n");
...
//					"-y", y,
					...
					"-o", "D:/FASTminDraw/"+y+"/" + lf[i].getName()
			};
			AppendToFile.appendMethodA("D:/FASTminDraw/"+y+"/T.txt", lf[i].getName());
			long st =System.currentTimeMillis();
			runFilter(new FASTminDraw(yuzhi), arg);
			long end =System.currentTimeMillis();
			long time=end-st;
			AppendToFile.appendMethodA("D:/FASTminDraw/"+y+"/T.txt", time + "\r\n");
		}
	}
}
