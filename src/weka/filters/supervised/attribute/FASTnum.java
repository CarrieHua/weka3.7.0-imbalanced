 ...
 */
public class FASTnum extends Filter implements SupervisedFilter, OptionHandler {
...
//					father[jj] = jj;
	...
	public boolean batchFinished() throws Exception {
		...
		if (!isOutputFormatDefined()) {
			...
			njjData = getInputFormat(); //input the train dataset
			int feature =njjData.numAttributes();
			Instances njjData1;
...
//			AppendToFile.appendMethodA("result/NJJ/Time.txt", ","+clusterNum+","+data.numAttributes()+","+njjData.numAttributes()+",");
			...
			m_OutputFormat.setRelationName(clusterNum+",-7-,"+ njjData.numAttributes()+ "," +name  );
			// Convert pending input instances
			for (int i = 0; i <njjData.numInstances(); i++) {
				convertInstancenjj(njjData.instance(i), GG, clusterNum);
			}
			AppendToFile.appendMethodA("D:/njj/Time7.txt", ","+clusterNum+",");
...
//		setSearch(new BestFirst());
	...
	public static void main(String[] argv) {
		...
		AppendToFile.appendMethodB("D:/njj/Time7.txt", " ");
...
//					"-S", "weka.attributeSelection.BestFirst -S 8",
					...
					"-o", "D:/njj/" + lf[i].getName()
			};
			AppendToFile.appendMethodA("D:/njj/Time7.txt", lf[i].getName());
			long st =System.currentTimeMillis();
			runFilter(new FASTnum(), arg);
			long end =System.currentTimeMillis();
			long time=end-st;
			AppendToFile.appendMethodA("D:/njj/Time7.txt", time + "\r\n");
		}
	}
