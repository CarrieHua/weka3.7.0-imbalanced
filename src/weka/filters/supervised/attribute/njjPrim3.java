 ...
 */
public class njjPrim3 extends Filter implements SupervisedFilter, OptionHandler {
	...
	public void njjcorrelationQ(Instances njjData, double su[][],double suic[]) {
		CfsSubsetEval njjCfs = new CfsSubsetEval();
		njjCfs.njjsetm_trInstances(njjData);
		...
		for (int i = 0; i < njjData.numAttributes()-1; i++) {
			for (int c = 0; c < num; c++) {
				ran = (int) (Math.random() * (njjData.numAttributes()-1));
	...
	public boolean batchFinished() throws Exception {
		...
		if (!isOutputFormatDefined()) {
			...
			double suij[][] = new double[njjFeatureNum][njjFeatureNum];			
			double[] suic=new double[njjFeatureNum+1];
				
			
			njjcorrelationQ(njjData, suij ,suic);
			...
			SUic=Sum/njjFeatureNum;
			
			int num=(int)Math.sqrt(njjFeatureNum);
			Sum=0;
			CfsSubsetEval njjCfs = new CfsSubsetEval();
			njjCfs.njjsetm_trInstances(njjData);				
			for(int i=0;i<num;i++){
				int j =(int)(Math.random()*njjFeatureNum);
				int c = (int)(Math.random()*njjFeatureNum);
				Sum+=njjCfs.njjsymmUncertCorr(j, c);			
			}
			SUij= Sum/num;
			
//			double m_Th=0.09935+0.82895*SUic;
			
			
			njjGraph1 njjg = new njjGraph1();
			njjGraph1 tree = njjg.njjprimsu(suij, 0);	
//			tree.displayAdjList();
			
			boolean[] visited=new boolean [tree.numVertex];
			int[]vclu=new int[njjFeatureNum];//
			int[]clu =new int [njjFeatureNum];
			int GG[] = new int[tree.numVertex + 1];	
			int clusterNum = 0;		
			
			for (int i = 0; i < njjFeatureNum; i++) { // 1
				if (!visited[i]) { // 2
					visited[i] = true;
					vclu[i] = i;
					if (suic[i] > m_Th) {
				...
				for (int j = 0; j < tree.adjList.elementAt(i).size(); j++) { // 3
					if (!visited[tree.adjList.elementAt(i).elementAt(j).v2]) { // 4
						visited[tree.adjList.elementAt(i).elementAt(j).v2] = true;
						if (suij[vclu[i]][tree.adjList.elementAt(i)
								.elementAt(j).v2] < Thij) { // 5
							vclu[tree.adjList.elementAt(i).elementAt(j).v2] = tree.adjList
									.elementAt(i).elementAt(j).v2;
							if (suic[tree.adjList.elementAt(i).elementAt(j).v2] > m_Th) {
								GG[clusterNum] = tree.adjList.elementAt(i)
										.elementAt(j).v2;
								clu[tree.adjList.elementAt(i).elementAt(j).v2] = clusterNum;
								clusterNum++;
							} else
								clu[tree.adjList.elementAt(i).elementAt(j).v2] = -1;
						}// 5/
						else {// 5
							if (clu[i] == -1) {// 6
								vclu[tree.adjList.elementAt(i).elementAt(j).v2] = tree.adjList
								...
								if (suic[tree.adjList.elementAt(i).elementAt(j).v2] > suic[GG[clu[i]]]) {
									GG[clu[i]] = tree.adjList.elementAt(i)
											.elementAt(j).v2;
									vclu[tree.adjList.elementAt(i).elementAt(j).v2] = tree.adjList
											.elementAt(i).elementAt(j).v2;
									clu[tree.adjList.elementAt(i).elementAt(j).v2] = clu[i];
								} else {
									vclu[tree.adjList.elementAt(i).elementAt(j).v2] = vclu[i];
									clu[tree.adjList.elementAt(i).elementAt(j).v2] = -1;
								}
							}// 6/
						}// 5/
			...
			long time = end - st;
			AppendToFile.appendMethodA("D:/exp/njj/njjPrim3.txt", m_Th+","+Thij+","+ SUic+","+SUij+","+clusterNum+","+njjData.numAttributes()+ ","+time + "\r\n");
			System.out.println("totall : \t" + (end - st));
			// *****************************
			GG[clusterNum] = njjFeatureNum;
			setOutputFormatnjj(GG, clusterNum);
			String name = njjData.relationName();
			m_OutputFormat.setRelationName(m_Th+","+Thij+","+ clusterNum+","+ njjData.numAttributes()+ ","  +name  );
			// Convert pending input instances
			for (int i = 0; i < getInputFormat().numInstances(); i++) {
				convertInstancenjj(getInputFormat().instance(i), GG, clusterNum);
			}
			flushInput();
		}
		m_NewBatch = true;
...
//		return (numPendingOutput() != 0);
	...
	public static void main(String[] argv) {
		...
		AppendToFile.appendMethodB("D:/exp/njj/njjPrim3.txt", " ");
		File[] lf = file.listFiles();
		for (int i = 0; i < lf.length; i++) {
			System.out.println(lf[i].getName());
			AppendToFile.appendMethodA("D:/exp/njj/njjPrim3.txt", lf[i].getName()
					+ ",\t");
				for (int p =0; p < 100; p = p +10) {// the big the more
					for (int k = 12; k < 31; k = k + 2) {// the little the more
					String[] arg = {
							"-E",
							"weka.attributeSelection.CfsSubsetEval -L",
							"-S",
							...
							String.valueOf(p),
							"-i",
							"D:/exp/njj/dataset/" + lf[i].getName(),
							"-o",
							"D:/exp/njj/newdataset/" + k + "_" + p + "_"
									+ lf[i].getName()
					// "-o", "D:/exp/njj/newdataset/"+"__"+ lf[i].getName()
					};
					runFilter(new njjPrim3(), arg);
				}
			}
		}
	}
