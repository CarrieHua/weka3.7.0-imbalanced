 ...
 */
public class njjsqre extends Filter implements SupervisedFilter, OptionHandler {
...
//		 setOptions[current++] = "" + getThij();
	...
	public void njjcorrelationQ(Instances njjData, double su[][],int[]ranked,int yuzhi) {
		CfsSubsetEval njjCfs = new CfsSubsetEval();
		njjCfs.njjsetm_trInstances(njjData);
		int cc =(int)Math.sqrt(yuzhi);
		int ran;
		for (int i = 0; i < yuzhi; i++) {
			for(int j=0;j<cc;j++){
				ran =(int) (Math.random()*yuzhi);
	...
	public void njjcorrelationQ2(Instances njjData, double su[][],int[]ranked,int yuzhi) {
		CfsSubsetEval njjCfs = new CfsSubsetEval();
		njjCfs.njjsetm_trInstances(njjData);
		int ran;
		for (int i = 0; i < yuzhi; i++) {
...
//					father[jj] = jj;
	...
	public boolean batchFinished() throws Exception {
		...
		if (!isOutputFormatDefined()) {
			...
			System.out.print(String.valueOf(end1-st1)+"\n");
...
//			long st=System.currentTimeMillis();
			njjcorrelationQ2(njjData,suij,ranked,yuzhi);	   //Complete graph
//			long end = System.currentTimeMillis();
//			System.out.print(String.valueOf(end-st)+"\n");
...
//			AppendToFile.appendMethodA("result/njjsqre/Time.txt", data.numInstances()+","+clusterNum+","+data.numAttributes()+","+njjData.numAttributes()+",");
			flushInput();
		}
		m_NewBatch = true;
	...
	public static void main(String[] argv) {
			...
			String[] arg = {
					...
					"-i", "dataset/" + argv[0], 
					"-o", "result/njjsqre/" + argv[0]
			};

			long st =System.currentTimeMillis();
			runFilter(new njjsqre(), arg);
			long end =System.currentTimeMillis();
			long time=end-st;
			AppendToFile.appendMethodA("result/njjsqre/Time1.txt",argv[0]+","+ time + "\r\n");
