...
//      m_OutputFormat.setRelationName( clusterNum+","+ njjData.numAttributes()+ ","  +name  );
      ...
      m_OutputFormat.setRelationName(m_SelectedAttributes.length+","+ getInputFormat().numAttributes()+ ",FCBF,"  +name  );
      for (int i = 0; i < getInputFormat().numInstances(); i++) {
	convertInstance(getInputFormat().instance(i));
      }
  	AppendToFile.appendMethodA("D:/7/FCBF/Time.txt", m_SelectedAttributes.length+","+getInputFormat().numAttributes()+",");
      flushInput();
    }
...
//		System.out.println("FCBF");
	  File file = new File("D:/dataset/");
		...
		double nnn=(double)NN/5.0;
		for(int ii = 1;ii<=4;ii++){
		int n =(int) nnn*ii;
		String Num = String.valueOf(n);
		for (int i = 0; i < 1; i++) {
			System.out.println(lf[i].getName());
			AppendToFile.appendMethodA("D:/7/FCBF/Time.txt",Num+ "_"+ lf[i].getName()+",");

		String[] arg = {
				"-E","weka.attributeSelection.SymmetricalUncertAttributeSetEval",
				"-S","weka.attributeSelection.FCBFSearch -D false -T -1 -N "+Num,
				"-c", "last",

				"-i", "D:/dataset/" +  lf[i].getName(), 
				"-o", "D:/7/FCBF/" +  Num+ "_"+ lf[i].getName()
				};
//		AppendToFile.appendMethodA("result/CFS/Time.txt", argv[0]);
		
		long st =System.currentTimeMillis();
		runFilter(new FCBFbest(), arg);
		long end =System.currentTimeMillis();
		long time=end-st;
		AppendToFile.appendMethodA("D:/7/FCBF/Time.txt", time + "\r\n");
  }
}
}
}
