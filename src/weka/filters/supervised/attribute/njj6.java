 ...
 */
public class njj6 extends Filter implements SupervisedFilter, OptionHandler {
...
//		 setOptions[current++] = "" + getThij();
	...
	public void njjcorrelationQ(Instances njjData, double su[][],int[]ranked,int yuzhi) {
		...
		int num = (int)Math.sqrt(yuzhi);
		int ran;
		for (int i = 0; i < yuzhi; i++) {
			for (int c = 0; c < num; c++) {
				ran =(int) (Math.random() * (yuzhi-1));
...
//						father[jj] = father[i];
	...
	public boolean batchFinished() throws Exception {
		...
		if (!isOutputFormatDefined()) {
			...
			njjData1 = Filter.useFilter(data, m_Filter);
			long sts = System.currentTimeMillis();
			
			Filter m_Filter1 = new weka.filters.unsupervised.attribute.RemoveUseless();
			 m_Filter1.setInputFormat(njjData1); 
			njjData = Filter.useFilter(njjData1, m_Filter1);
			
			int njjFeatureNum = njjData.numAttributes() - 1;
			
			int yuzhi= (int)(njjFeatureNum/(Math.log(njjFeatureNum)));
			suic=new double[njjFeatureNum];
//			double[] mpp=new double[njjFeatureNum];		
			SUIC(njjData,suic);
			
//			double[]suic ={0.3,0.2,0.5};

			int[] ranked = Utils.sort(suic);
//			System.out.println(ranked[0]);
			double m_Th=suic[ranked[yuzhi]];

			
			suij = new double[yuzhi][yuzhi];	
			njjcorrelationQ(njjData,suij,ranked,yuzhi);

		
			njjGraph1 njjg = new njjGraph1();
			double[] a=new double[yuzhi];
			double[] sum=new double [0];
			njjGraph1 tree = njjg.njjprimsu5(suij, 0,a,sum);	
			int ak=(int)(yuzhi*m_Th*0.618);
			int[] rank=Utils.sort(a);
//			System.out.print(yuzhi+"\t");
//			System.out.print(ak+"\t");
			
//			System.out.print(m_Th/30+"\t");
			
			double Thij=a[rank[ak]];
...
//			double Thij=m_Th/10;
			...
			AppendToFile.appendMethodA("D:/exp/njj/njjPrim6.txt", name+","+clusterNum+","+feature+ ","+time + ","+timet+"\r\n");
			System.out.println("totall : \t" + (end - st));
			// *****************************
			GG[clusterNum] = njjFeatureNum;
			setOutputFormatnjj(GG, clusterNum);
		
			m_OutputFormat.setRelationName( clusterNum+","+ feature+ ","  +name  );
...
//		File file = new File("D:/exp/njj/bigDSlala/");
		File file = new File("D:/exp/njj/dataset/");
		
		AppendToFile.appendMethodB("D:/exp/njj/njjPrim6.txt", " ");
		File[] lf = file.listFiles();
		for (int i = 0; i < lf.length; i++) {
			System.out.println(lf[i].getName());

			String[] arg = { "-E", "weka.attributeSelection.CfsSubsetEval -L",
					"-S", "weka.attributeSelection.BestFirst -S 8",
					"-c", "last",
//					"-i", "D:/exp/njj/bigDSlala/" + lf[i].getName(), "-o",
					"-i", "D:/exp/njj/dataset/" + lf[i].getName(), "-o",
					
//					"D:/exp/njj/NJJbigDS/" + lf[i].getName()
					"D:/exp/njj/newdataset/" + lf[i].getName()
			};
			runFilter(new njj6(), arg);
		}
	}
}
