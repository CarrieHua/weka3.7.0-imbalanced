 ...
 */
public class njjRandom extends Filter implements SupervisedFilter, OptionHandler {
	...
	public void njjcorrelationQ(Instances njjData, float su[][],float suic[]) {
		CfsSubsetEval njjCfs = new CfsSubsetEval();
		njjCfs.njjsetm_trInstances(njjData);
//		int num = njjData.numAttributes() / 25;
		int num = 
			(int)Math.sqrt(njjData.numAttributes());
//		if(su.length>5000)
		...
		for (int i = 0; i < njjData.numAttributes()-1; i++) {
			for (int c = 0; c < num; c++) {
				...
				su[i][ran] =(float)njjCfs.njjsymmUncertCorr(i, ran);
				su[ran][i]=su[i][ran];
			}
		}
		int Cno=njjData.numAttributes()-1;
		for (int i = 0; i < njjData.numAttributes()-1; i++) {
			suic[i+1] = (float)njjCfs.njjsymmUncertCorr(i, Cno);
		}
	}
	
	public double SU(Instances njjData,int i, int j){
		CfsSubsetEval njjCfs = new CfsSubsetEval();
		njjCfs.njjsetm_trInstances(njjData);
		
		double su= njjCfs.njjsymmUncertCorr(i, j);
		return su;
	}
	
	public void feedSU(Instances njjData, double su[][],double suic[]) {
		CfsSubsetEval njjCfs = new CfsSubsetEval();
		njjCfs.njjsetm_trInstances(njjData);
		int duan=su.length;
		int duanchang=(njjData.numAttributes()-1)/duan+1;
		int I,J;
		int i;
		
		for( i=0;i<duan-1;i++){
			I=i*duanchang;
			for(int j=1;j<duanchang;j++){
				J=I+j;
				su[i][j]= njjCfs.njjsymmUncertCorr(I, J);
			}
		}
		...
		int Cno=njjData.numAttributes()-1;
		for ( i = 0; i < njjData.numAttributes()-1; i++) {
	...
	public boolean batchFinished() throws Exception {
		...
		if (!isOutputFormatDefined()) {
			...
			int featureNum = njjData.numAttributes() - 1;

//			int duan = (int)Math.sqrt(njjData.numAttributes());
//			
//			int duanchang = featureNum / duan + 1;
//
//			double SUij[][] = new double[duan][duanchang];
			double SUic[] = new double[njjData.numAttributes()];
//			SUic[0]=-1;
			CfsSubsetEval njjCfs = new CfsSubsetEval();
			njjCfs.njjsetm_trInstances(njjData);
			for(int i=0;i<featureNum;i++){
				SUic[i]=njjCfs.njjsymmUncertCorr(i, featureNum);
			}
			
			int icNum= (int)Math.sqrt(featureNum);
			
			double SUSum=0;
			for(int i=0;i<icNum;i++){
				int j =(int)(Math.random()*featureNum);
				SUSum+=SUic[j];
			}
			double m_Th=SUSum/icNum*1.5;
			
			SUSum=0;
			int ijNum = (int)Math.sqrt(featureNum);
			for(int i=0;i<ijNum;i++){
				int j =(int)(Math.random()*featureNum);
				int c = (int)(Math.random()*featureNum);
				SUSum+=SU(njjData,j,c);
			}
			double Thij=SUSum/ijNum;
//
//			feedSU(njjData, SUij, SUic);


			int GG[] = new int[featureNum + 1];
			int clusterNum = 0;
	//		Thij = 0.15;
			...
			for (Index++; Index < featureNum; Index++) {
				if (SU(njjData, Index, clusterNum - 1) >= Thij) {
					if (SUic[Index] > SUic[clusterNum - 1])
						GG[clusterNum - 1] = Index;
				} else if (SUic[Index + 1] > m_Th) {
					GG[clusterNum] = Index;
					clusterNum++;
				}
			}
...
//				for (int j = 1; j < duanchang; j++) {
//					J = I + j;
//					if (SUij[i][j] < Thij && SUic[1 + J] > m_Th ) {			
//						GG[clusterNum] = J;
//						clusterNum++;
//					} else if(SUij[i][j] >= Thij && SUic[1 + J] > SUic[GG[clusterNum-1]] && SUic[1 + J] > m_Th)
//						GG[clusterNum-1]=J;
//					
//				}
//			}
...
//			m_OutputFormat.setRelationName(m_Th + "," + Thij + "," + name + ","
//					+ clusterNum + "," + njjData.numAttributes());
//			for (int i = 0; i < njjnewData.numInstances(); i++) {
...
//			else {
			...
			/*
				 for(int p=30;p<31;p=p+10){//the big the more
					 for(int k=30;k<31;k=k+20){// the little the more
*/				// for(int k=2;k<=9;k++){
				String[] arg = { 
...
//						"-o", "D:/exp/njj/newdataset/"+k+"R4_"+p+"_"+ lf[i].getName()	
						"-o", "D:/exp/njj/newdataset/"+"_1__"+ lf[i].getName()	
						};
				runFilter(new njjRandom(), arg);
//			}
		}//}
	}
//				 }}
