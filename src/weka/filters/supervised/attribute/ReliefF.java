...
public class ReliefF
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    AttrNum=getInputFormat().numAttributes();
    String Njj=String.valueOf(N*(double)AttrNum);
    optionString = Utils.getOption('S',options)+" -N "+Njj;
...
//      m_OutputFormat.setRelationName( clusterNum+","+ njjData.numAttributes()+ ","  +name  );
	...
	convertInstance(getInputFormat().instance(i));
	  ...
	  AppendToFile.appendMethodA("D:/7/ReliefF/Time.txt",","+m_SelectedAttributes.length+","+getInputFormat().numAttributes()+",");
      flushInput();
    }
...
//		AppendToFile.appendMethodB("D:/result/ReliefF/Time.txt", " ");
		File[] lf = file.listFiles();
		for (int i = 0; i < lf.length; i++) {
			for(int ii=1;ii<=4;ii++){
				double n= 0.2*ii;
				String N= String.valueOf(n);
				AppendToFile.appendMethodA("D:/7/ReliefF/Time.txt",N+ "_"+ lf[i].getName()+",");
		String[] arg = {
				"-E","weka.attributeSelection.ReliefFAttributeEval -M -1 -D 1 -K 10",
				"-S","weka.attributeSelection.Ranker -T 0.003 -N -1",
//				-N -1",
				"-c", "last",

				"-i", "D:/dataset/" + lf[i].getName(), 
				"-o", "D:/7/ReliefF/" + N+"_"+  lf[i].getName()
		};	
//		AppendToFile.appendMethodA("result/ReliefF/Time.txt", argv[0]);
		long st =System.currentTimeMillis();
		runFilter(new ReliefF(), arg);
		long end =System.currentTimeMillis();
		long time=end-st;
