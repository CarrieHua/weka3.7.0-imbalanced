...
public class Fen4k extends Filter implements SupervisedFilter, OptionHandler {
	...
	public boolean batchFinished() throws Exception {
		...
		if (!isOutputFormatDefined()) {
			...
			Instances data1 = getInputFormat();  //input the train data set
			...
			AppendToFile.appendMethodA("D:/fen4kResult/Time.txt", data.numInstances()+","+N+",");
			...
			boolean [] choseSeeds = new boolean[N];
			
//----Nominal data set			
			if((CountNum < x || CountNum < S) && (CountNom >= x && CountNom >= S)){
				
				AppendToFile.appendMethodA("D:/fen4kResult/Time.txt", "Nom"+","+S+","+x+",");
...
//-------------------------------------------------------------------------------
				...
				AppendToFile.appendMethodA("D:/fen4kResult/Time.txt", "Num"+","+S+","+x+",");
...
//-------------------------------------------------------------------
				...
				AppendToFile.appendMethodA("D:/fen4kResult/Time.txt", "Mix"+","+S+","+x+",");
				yNom = (CountNom-1)/x + 1;
				...
				AppendToFile.appendMethodA("D:/fen4kResult/Time.txt", "FALSE!"+",");
				return false;
			}
			...
			m_OutputFormat.setRelationName(y+",Fen4k,"+ data.numAttributes()+ "," +data.relationName() );
			// Convert pending input instances
			for (int i = 0; i <data.numInstances(); i++) {
				convertInstancenjj(data.instance(i), GG, y);
			}
			AppendToFile.appendMethodA("D:/fen4kResult/Time.txt", y+",");
...
//		setSearch(new BestFirst());
	...
	public static void main(String[] argv) {
		...
		AppendToFile.appendMethodB("D:/fen4kResult/Time.txt", "name,\tI,\tF,\tcatg,\tS,\tx,\tnewF,\tFenTime\r\n");
...
//					"-S", "weka.attributeSelection.BestFirst -S 8",
					...
					"-o", "D:/fen4kResult/" + lf[i].getName()
			};
			AppendToFile.appendMethodA("D:/fen4kResult/Time.txt", lf[i].getName()+",");
			long st =System.currentTimeMillis();
			runFilter(new Fen4k(), arg);
			long end =System.currentTimeMillis();
			long time=end-st;
			AppendToFile.appendMethodA("D:/fen4kResult/Time.txt", time + "\r\n");
		}
	}
