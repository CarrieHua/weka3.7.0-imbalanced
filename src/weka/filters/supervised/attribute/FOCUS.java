...
//      m_OutputFormat.setRelationName( clusterNum+","+ njjData.numAttributes()+ ","  +name  );
      ...
      m_OutputFormat.setRelationName(m_SelectedAttributes.length+","+ getInputFormat().numAttributes()+ ",CFS"  +name  );
      for (int i = 0; i < getInputFormat().numInstances(); i++) {
	convertInstance(getInputFormat().instance(i));
      }
  	AppendToFile.appendMethodA("result/FOCUS/Time.txt", getInputFormat().numInstances()+","+m_SelectedAttributes.length+","+getInputFormat().numAttributes()+","+",");
      flushInput();
    }
  ...
  public static void main(String [] argv) {
		...
		String[] arg = {
				"-E","weka.attributeSelection.ConsistencySubsetEval",
				"-S","weka.attributeSelection.GreedyStepwise -T -1.7976931348623157E308 -N -1",
				"-c", "last",

				"-i", "dataset/" + argv[0], 
				"-o", "result/FOCUS/" + argv[0]
		};
//		AppendToFile.appendMethodA("result/CFS/Time.txt", argv[0]);
		long st =System.currentTimeMillis();
		runFilter(new FOCUS(), arg);
		long end =System.currentTimeMillis();
		long time=end-st;
		AppendToFile.appendMethodA("result/FOCUS/Time.txt", time + "\r\n");
  }
}
