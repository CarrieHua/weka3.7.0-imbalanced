...
//      m_OutputFormat.setRelationName( clusterNum+","+ njjData.numAttributes()+ ","  +name  );
	...
	convertInstance(getInputFormat().instance(i));
  	...
  	AppendToFile.appendMethodA("result/Consist/Time.txt", getInputFormat().numInstances()+","+m_SelectedAttributes.length+","+getInputFormat().numAttributes()+","+",");
      flushInput();
    }
  ...
  public static void main(String [] argv) {
		...
		String[] arg = {
				...
				"-i", "dataset/" + argv[0], 
				"-o", "result/Consist/" + argv[0]
		};
//		AppendToFile.appendMethodA("result/CFS/Time.txt", argv[0]);
		long st =System.currentTimeMillis();
		runFilter(new Consist(), arg);
		long end =System.currentTimeMillis();
		long time=end-st;
		AppendToFile.appendMethodA("result/Consist/Time.txt", time + "\r\n");
  }
}
