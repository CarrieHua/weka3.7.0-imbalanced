 ...
 */
public class FASTdef6 extends Filter implements SupervisedFilter, OptionHandler {
...
//					father[jj] = jj;
	...
	public void BFS2(int i, njjGraph1 tree,  int[] ranked, double[] suic) {
		for (int j = 0; j < tree.adjList.elementAt(i).size(); j++) { // 3
			int jj = tree.adjList.elementAt(i).elementAt(j).v2;
			if (!visited[jj]) { // 4
				visited[jj] = true;
//				if (suij[i][jj] < Thij) {
				...
				if (suij[i][jj] < suic[i]||suij[i][jj]< suic[jj]) {					
					GG[clusterNum] = ranked[jj];
					clu[jj] = clusterNum;
					clusterNum++;
//					father[jj] = jj;
				...
				BFS2(jj, tree, ranked, suic);
			}
		}// 3/
	}
	
	public  void BFS3(int i, njjGraph1 tree,  int[] ranked, double[] suic) {
		visited[i]=true;
		double suij;
		...
		for (int j = 0; j < tree.adjList.elementAt(i).size(); j++) { // 3
			...
			if (!visited[jj]) { // 4
				visited[jj] = true;
				if (suij > suic[i] && suij> suic[jj]) {	
					if(suic[i]>suic[jj]){
						GG[clusterNum]=ranked[jj];
//						System.out.print(ranked[jj]+"\t");
						}
					else{
						GG[clusterNum]= ranked[i];
//						System.out.print(ranked[jj]+"\t");
						}
...
//			BFS(0,tree,Thij,ranked);
			...
			BFS3(0,tree,ranked,suic);
			
			
			
//			int[] rankedGG = Utils.sort(GG); 
...
//			}
			...
			int[] GGG = new int [yuzhi+1];
			GGG = removeFrom(yuzhiAtr,GG);
			...
			setOutputFormatnjj(GGG, sf);
		
			m_OutputFormat.setRelationName("BFS3,"+ data.relationName()+","+yuzhi1SU+","  );
			for (int i = 0; i <njjData.numInstances(); i++) {
				convertInstancenjj(njjData.instance(i), GGG, sf);
			}
			AppendToFile.appendMethodA("D:/BFS3/Time.txt", ","+data.relationName()+","+data.numAttributes()+","+njjData.numAttributes()+","+yuzhi+","+sf+","+data.numInstances()+","+data.numClasses()+","+yuzhi1SU+",");
...
//		setSearch(new BestFirst());
	...
	public static void main(String[] argv) {
		File file = new File("D:/BFS3/dataset/");
		AppendToFile.appendMethodB("D:/BFS3/Time.txt", " ");
...
//					"-S", "weka.attributeSelection.BestFirst -S 8",
					...
					"-i", "D:/BFS3/dataset/" + lf[i].getName(), 
					"-o", "D:/BFS3/" + lf[i].getName()
			};
			AppendToFile.appendMethodA("D:/BFS3/Time.txt", lf[i].getName());
			long st =System.currentTimeMillis();
			runFilter(new FASTdef6(), arg);
			long end =System.currentTimeMillis();
			long time=end-st;
			AppendToFile.appendMethodA("D:/BFS3/Time.txt", time + "\r\n");
		}
	}
