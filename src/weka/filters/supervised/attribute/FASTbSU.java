 ...
 */
public class FASTbSU extends Filter implements SupervisedFilter, OptionHandler {
...
//			}
			...
			m_OutputFormat.setRelationName("FASTbSU,"+ data.relationName()+","+yuzhi1SU+","  );
			for (int i = 0; i <njjData.numInstances(); i++) {
				convertInstancenjj(njjData.instance(i), retained, sf);
			}
			AppendToFile.appendMethodA("D:/FASTbSU/Time.txt", ","+data.relationName()+","+data.numAttributes()+","+njjData.numAttributes()+","+yuzhi+","+sf+","+data.numInstances()+","+data.numClasses()+","+yuzhi1SU+",");
...
//		setSearch(new BestFirst());
	...
	public static void main(String[] argv) {
		...
		AppendToFile.appendMethodB("D:/FASTbSU/Time.txt", " ");
...
//					"-S", "weka.attributeSelection.BestFirst -S 8",
					...
					"-o", "D:/FASTbSU/" + lf[i].getName()
			};
			AppendToFile.appendMethodA("D:/FASTbSU/Time.txt", lf[i].getName());
			long st =System.currentTimeMillis();
			runFilter(new FASTbSU(), arg);
			long end =System.currentTimeMillis();
			long time=end-st;
			AppendToFile.appendMethodA("D:/FASTbSU/Time.txt", time + "\r\n");
		}
	}
