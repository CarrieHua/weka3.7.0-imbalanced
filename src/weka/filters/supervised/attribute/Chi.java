...
//      m_OutputFormat.setRelationName( clusterNum+","+ njjData.numAttributes()+ ","  +name  );
	...
	convertInstance(getInputFormat().instance(i));
  	...
  	AppendToFile.appendMethodA("result/Chi/Time.txt", getInputFormat().numInstances()+","+m_SelectedAttributes.length+","+getInputFormat().numAttributes()+","+",");
      flushInput();
    }
...
//		AppendToFile.appendMethodA("result/CFS/Time.txt", argv[0]);
		long st =System.currentTimeMillis();
		runFilter(new Chi(), arg);
		long end =System.currentTimeMillis();
		long time=end-st;
		AppendToFile.appendMethodA("result/Chi/Time.txt", time + "\r\n");
  }
}
