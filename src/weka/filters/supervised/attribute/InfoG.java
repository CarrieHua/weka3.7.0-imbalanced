...
//      m_OutputFormat.setRelationName( clusterNum+","+ njjData.numAttributes()+ ","  +name  );
	...
	convertInstance(getInputFormat().instance(i));
  	...
  	AppendToFile.appendMethodA("result/InfoG/Time.txt", getInputFormat().numInstances()+","+m_SelectedAttributes.length+","+getInputFormat().numAttributes()+","+",");
      flushInput();
    }
...
//		System.out.println("Chi");
		String[] arg = {
				...
				"-S","weka.attributeSelection.Ranker -T 0.5 -N -1",
				"-c", "last",

				"-i", "dataset/" + argv[0], 
				"-o", "result/InfoG/" + argv[0]
		};
//		AppendToFile.appendMethodA("result/CFS/Time.txt", argv[0]);
		long st =System.currentTimeMillis();
		runFilter(new InfoG(), arg);
		long end =System.currentTimeMillis();
		long time=end-st;
		AppendToFile.appendMethodA("result/InfoG/Time.txt", time + "\r\n");
  }
}
