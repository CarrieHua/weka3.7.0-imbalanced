 ...
 */
public class FAST10d extends Filter implements SupervisedFilter, OptionHandler {
...
//					father[jj] = jj;
	...
	public void BFS2(int i, njjGraph1 tree,  int[] ranked, double[] suic) {
		for (int j = 0; j < tree.adjList.elementAt(i).size(); j++) { // 3
			...
			if (!visited[jj]) { // 4
				visited[jj] = true;
				if (suij[i][jj] < suic[ranked[i]]||suij[i][jj]< suic[ranked[jj]]) {		
					GG[clusterNum] = ranked[jj];
					clu[jj] = clusterNum;
					clusterNum++;
//					father[jj] = jj;
...
//			System.out.print(suic[ranked[0]]+"\t"+suic[ranked[1]]+"\t"+suic[ranked[2]]+"\t");
//			System.out.print(suic[ranked[njjFeatureNum-3]]+"\t"+suic[ranked[njjFeatureNum-2]]+"\t"+suic[ranked[njjFeatureNum-1]]+"\t");
...
//			njjGraph1 tree = njjg.njjprimsu55(suij, 0,a);	
...
//			AppendToFile.appendMethodA("result/NJJ/Time.txt", ","+clusterNum+","+data.numAttributes()+","+njjData.numAttributes()+",");
			...
			m_OutputFormat.setRelationName("FAST10d,"+ data.relationName()+","+yuzhi1SU+","  );
			// Convert pending input instances
			for (int i = 0; i <njjData.numInstances(); i++) {
				convertInstancenjj(njjData.instance(i), GG, clusterNum);
			}
			AppendToFile.appendMethodA("D:/FAST10d/Time.txt", ","+data.relationName()+","+data.numAttributes()+","+njjData.numAttributes()+","+clusterNum+","+data.numInstances()+","+data.numClasses()+","+yuzhi1SU+",");
...
//		setSearch(new BestFirst());
	...
	public static void main(String[] argv) {
		...
		AppendToFile.appendMethodB("D:/FAST10d/Time.txt", " ");
...
//					"-S", "weka.attributeSelection.BestFirst -S 8",
					...
					"-o", "D:/FAST10d/" + lf[i].getName()
			};
			AppendToFile.appendMethodA("D:/FAST10d/Time.txt", lf[i].getName());
			long st =System.currentTimeMillis();
			runFilter(new FAST10d(), arg);
			long end =System.currentTimeMillis();
			long time=end-st;
			AppendToFile.appendMethodA("D:/FAST10d/Time.txt", time + "\r\n");
		}
	}
