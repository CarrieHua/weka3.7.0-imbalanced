 ...
 */
public class FASTdef6c extends Filter implements SupervisedFilter, OptionHandler {
	...
	private int cluNum=1;

	private double [][] Single;
	private double minsu=0;
	private double sumsu=0;
...
//	}
	...
	public  void BFS5(int i, njjGraph1 tree,  int[] ranked) {
		visited[i]=true;
		double suij;
		clu[i][0]=cluNum;
	...
	public  void single(int[] ranked,int yuzhi){
		int[][] cluster=new int[cluNum+1][yuzhi];
		for(int i=0;i<cluNum+1;i++)
			for(int j=1;j<yuzhi;j++)
				cluster[i][j]=-1;
...
//			System.out.print("\n");
			...
			double Rave= sum/(double)sff;
			
			for(int i=1;i<yuzhi;i++){
				if(eachCluNum[i]==1){
					sig++;
//					AppendToFile.appendMethodA("D:/FASTdef6c/su.txt", ","+cluster[i][0]+",:,"+suic[ranked[cluster[i][0]]]+",");
...
//			}
			...
			m_OutputFormat.setRelationName("FASTdef6c,"+ data.relationName()+","+yuzhi1SU+","  );
			for (int i = 0; i <njjData.numInstances(); i++) {
				convertInstancenjj(njjData.instance(i), retained, sf);
			}
			AppendToFile.appendMethodA("D:/FASTdef6c/Time.txt", ","+data.relationName()+","+data.numAttributes()+","+njjData.numAttributes()+","+yuzhi+","+sf+","+data.numInstances()+","+data.numClasses()+","+yuzhi1SU+",");
			AppendToFile.appendMethodA("D:/FASTdef6c/su.txt", ","+data.relationName()+","+data.numAttributes()+","+njjData.numAttributes()+","+yuzhi+","+sf+",");
			
			AppendToFile.appendMethodA("D:/FASTdef6c/su.txt","sig,"+sig+",yuzhi1SU,"+yuzhi1SU+",RaveSU,"+Rave);
			for(int i=1;i<yuzhi;i++){
				if(eachCluNum[i]==1){
...
//					System.out.print(cluster[i][0]+": "+suic[ranked[cluster[i][0]]]+"\t");
			...
			AppendToFile.appendMethodA("D:/FASTdef6c/su.txt","\r\n");
...
//		setSearch(new BestFirst());
	...
	public static void main(String[] argv) {
		...
		AppendToFile.appendMethodB("D:/FASTdef6c/Time.txt", " ");
		AppendToFile.appendMethodB("D:/FASTdef6c/su.txt", " ");
...
//					"-S", "weka.attributeSelection.BestFirst -S 8",
					...
					"-o", "D:/FASTdef6c/" + lf[i].getName()
			};
			AppendToFile.appendMethodA("D:/FASTdef6c/Time.txt", lf[i].getName());
			AppendToFile.appendMethodA("D:/FASTdef6c/su.txt", lf[i].getName());
			long st =System.currentTimeMillis();
			runFilter(new FASTdef6c(), arg);
			long end =System.currentTimeMillis();
			long time=end-st;
			AppendToFile.appendMethodA("D:/FASTdef6c/Time.txt", time + "\r\n");
		}
	}
