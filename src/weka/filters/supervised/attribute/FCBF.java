...
public class FCBF
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    optionString = Utils.getOption('S',options)+"-N 10";
    if (optionString.length() != 0) {
      optionString = optionString.trim();
      int breakLoc = optionString.indexOf(' ');
...
//      m_OutputFormat.setRelationName( clusterNum+","+ njjData.numAttributes()+ ","  +name  );
	...
	convertInstance(getInputFormat().instance(i));
  	...
  	AppendToFile.appendMethodA("D:/7/FCBF_Test/Time.txt", m_SelectedAttributes.length+","+getInputFormat().numAttributes()+",");
      flushInput();
    }
...
//		AppendToFile.appendMethodB("D:/NEXTaResult/Time.txt", "name,\tI,\tF,\tcatg,\tS,\tx,\tnewF,\tFenTime\r\n");
		...
		String[] arg = {
				...
				"-S","weka.attributeSelection.FCBFSearch -D false -T -1 ",
				"-c", "last",

				"-i", "D:/dataset/" +  lf[i].getName(), 
				"-o", "D:/7/FCBF_Test/" +  lf[i].getName()
		};
//		AppendToFile.appendMethodA("result/CFS/Time.txt", argv[0]);
		double nn=0.2;
		long st =System.currentTimeMillis();
		runFilter(new FCBF(nn), arg);
		long end =System.currentTimeMillis();
		long time=end-st;
		AppendToFile.appendMethodA("D:/7/FCBF_test/Time.txt", time + "\r\n");
  }
}
}
