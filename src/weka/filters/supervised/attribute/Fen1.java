 ...
 * only can deal with nominal data sets.
 */
 ...
 */
public class Fen1 extends Filter implements SupervisedFilter, OptionHandler {
...
//	}
	...
	public int CHOSE(int[] a,int x, Instances njjData){
		int b=0;
...
//			Instances data = Filter.useFilter(data1, m_Filter);  //discretize			
...
//						int a = seeds[ii][0];
			...
			for(int i= 0; i< (y-1); i++){
				fen[i][x]=CHOSE(fen[i],x,data);					
			}
			fen[y-1][x]= CHOSE(fen[y-1],weiba,data);
...
//			// *****************************
			...
			m_OutputFormat.setRelationName(y+",Fen1,"+ data.numAttributes()+ "," +data.relationName() );
			// Convert pending input instances
			for (int i = 0; i <data.numInstances(); i++) {
				convertInstancenjj(data.instance(i), GG, y);
			}
			AppendToFile.appendMethodA("D:/fen1Result/Time.txt", ","+y+",");
...
//		setSearch(new BestFirst());
	...
	public static void main(String[] argv) {
		...
		AppendToFile.appendMethodB("D:/fen1Result/Time.txt", " ");
...
//					"-S", "weka.attributeSelection.BestFirst -S 8",
					...
					"-o", "D:/fen1Result/" + lf[i].getName()
			};
			AppendToFile.appendMethodA("D:/fen1Result/Time.txt", lf[i].getName());
			long st =System.currentTimeMillis();
			runFilter(new Fen1(), arg);
			long end =System.currentTimeMillis();
			long time=end-st;
			AppendToFile.appendMethodA("D:/fen1Result/Time.txt", time + "\r\n");
		}
	}
