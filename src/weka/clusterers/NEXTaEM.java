...
public class NEXTaEM
 ...
 /** globally replace missing values */
   ...
   **/
  private void NEXTaEM_Init (Instances inst)
    throws Exception {
    int i, j, k;
...
//  private void EM_Report (Instances inst) {
  public void NEXTaEM_Report (Instances inst) {
    int i, j, l, m;
    System.out.println("======================================");
...
//  public static boolean isIncremental(String fileName) throws Exception {
...
//	}
  ...
  public static Instances generateData(String fileName){
...
//				m_Data.setClassIndex(m_Data.numAttributes()-1);
   ...
   public static double MaxInAtr(int a, Instances data){
	   double max=0;
	   for(int i=0;i<data.numInstances();){
		   if(Double.isNaN(data.instance(i).value(a)))
			   i++;
		   else{
			   max=data.instance(i).value(a);
			   break;
		   }
	   }
	   for(int i=0;i<data.numInstances();i++){
		   if(data.instance(i).value(a)>max)
			   max=data.instance(i).value(a);
	   }
	   return max;
   }
   
   public static double MinInAtr(int a, Instances data){
	   double min=100;
	   for(int i=0;i<data.numInstances();){
		   if(Double.isNaN(data.instance(i).value(a)))
			   i++;
		   else{
			   min=data.instance(i).value(a);
			   break;
		   }
	   }
	   for(int i=0;i<data.numInstances();i++){
		   if(data.instance(i).value(a)<min)
			   min=data.instance(i).value(a);
	   }
	   return min;
   }
   
   public static double[] GUIYI(Instances data) {
		double up = 0;
		double low = 0;
		int catg = 0;
		double[] guiyi = new double[data.numAttributes()];
		...
		for (int i = 0; i < data.numAttributes(); i++) {
			if (data.attribute(i).isNumeric()) {
				...
				catg = data.attribute(i).numValues();
				guiyi[i] = (double) catg;
			}
		}
		return guiyi;
   ...
   public static double dis(int a, int b, Instances data, double[] guiyi) {
//	   double up =0;
//	   double low =0;
//	   int catg = 0 ;
//	   double[] guiyi = new double[data.numAttributes()];
	   double[] zhi = new double[data.numAttributes()];
	   double temp;
	   	   
		for (int i = 0; i < data.numAttributes(); i++) {
			if (data.attribute(i).isNumeric()) {
//				up = data.attribute(i).getUpperNumericBound();
//				low = data.attribute(i).getLowerNumericBound();
//				guiyi[i]=up-low;
				double aV = data.instance(a).value(i);
				double bV = data.instance(b).value(i);
				...
				else if(Double.isNaN(aV) || Double.isNaN(bV)){
					zhi[i]= 0.5;
				}
				else{				
				temp = data.instance(a).value(i)-data.instance(b).value(i);
				if(temp<0)
					temp=-temp;
...
//				catg = data.attribute(i).numValues();
//				guiyi[i]= (double) catg;
//				String aV = data.instance(a).stringValue(i);
//				String bV = data.instance(b).stringValue(i);
//				if(aV==String.NaN && bV==String.NaN){
//					zhi[i]= 0;					
//				}
//				else if(aV==Double.NaN || bV==Double.NaN){
//					zhi[i]= 0.5;
//				}
//				else{	
				if(data.instance(a).stringValue(i).equals(data.instance(b).stringValue(i)))
//					zhi[i]= 1.0/guiyi[i];
					zhi[i]= 0;
				else
					zhi[i]= 1.0/guiyi[i];
//				}
		...
		temp=0;
		for(int i=0;i<data.numAttributes();i++)
			temp= temp+(zhi[i]*zhi[i]);
		return Math.sqrt(temp);
	}
   
   public static double aveInClu(int[] clu, Instances dataset,double[] guiyi){
	   double sum =0;
	   int E = clu[0];
	   if (E<=1)
		   return 0;
	   ...
	   for(int i = 1 ;i<=E;i++)
		   for(int j = i+1;j<=E;j++){
			   sum+= dis(clu[i],clu[j],dataset,guiyi);
		   }	   
	   return sum/(double)V;
   }
   
   public static double TwoE(int[] clu1, int[] clu2, Instances data,double[] guiyi){
	   double sum = 0;
	   int E1 = clu1[0];
	   int E2 = clu2[0];
	   if(E1>0 && E2 >0){
	   for(int i = 1 ;i<=E1;i++)
		   for(int j = 1;j<=E2;j++){
			   sum+= dis(clu1[i],clu2[j],data,guiyi);
		   }
	   }
	   return sum;
   }
  ...
  public static void main (String[] argv) throws Exception {
		...
		AppendToFile.appendMethodB("D:/NEXTaResult/EM.txt", "name,\tTime,\tNewTime\r\n");
		
		File file = new File("D:/datasetEM/");
		File[] lf = file.listFiles();
		for (int ii = 0; ii < lf.length; ii++) {
			System.out.print(lf[ii].getName()+"\t\t");
			AppendToFile.appendMethodA("D:/NEXTaResult/EM.txt", lf[ii].getName()+",");
			
			Instances dataset = generateData("D:/dataset/" + lf[ii].getName());
//			double bb = dataset.instance(0).value(0);
//			String aa0 = dataset.instance(0).stringValue(1);
//			double cc = dataset.instance(0).value(2);
//			String aa1 = dataset.instance(1).stringValue(1);
//			String aa2 = dataset.instance(2).stringValue(1);
			dataset.deleteAttributeAt(dataset.numAttributes()-1);
			dataset.setClassIndex(-1);
			
			double[] guiyi = new double[dataset.numAttributes()];
			guiyi = GUIYI(dataset);
			                            
			
		
			
			NEXTaEM EM1 = new NEXTaEM();
			
			int AA= dataset.numAttributes();
			int BB= dataset.numInstances();
			
			long st =System.currentTimeMillis();
			EM1.buildClusterer(dataset);
			long end =System.currentTimeMillis();
			
			long time=end-st;
			AppendToFile.appendMethodA("D:/NEXTaResult/EM.txt", time + ",");
			
			int InsNum = dataset.numInstances();
			int[][] clusters = new int[m_num_clusters][InsNum + 1];
			for (int i = 0; i < InsNum; i++) {
				int cluIdex = EM1.clusterInstance(dataset.instance(i));
				clusters[cluIdex][0]++;
				clusters[cluIdex][clusters[cluIdex][0]] = i;
			}
			System.out.print("\n");
			for (int i =0;i<m_num_clusters;i++){
			System.out.print(i+" \t"+(double)clusters[i][0]/(double)InsNum+"\n");
			}
			
			
			if(m_num_clusters<2)
				System.out.print("Only one cluster!\t");
			else{
			double[] aveIn = new double[m_num_clusters];
			for(int i = 0; i< m_num_clusters; i++){
				aveIn[i]= aveInClu(clusters[i], dataset, guiyi);
			}
			
			double aveInOne = ave(aveIn);
						
			int VE=0;
			double SumE = 0;
			for(int i=0; i<m_num_clusters;i++){
				for(int j=i+1;j<m_num_clusters;j++){
					VE=VE+ clusters[i][0]*clusters[j][0];
					SumE = SumE + TwoE(clusters[i],clusters[j],dataset, guiyi);					
				}
			}
			
			double aveEOne = SumE/(double)VE;
			
			double NJJ = aveEOne/aveInOne;
			
			System.out.print("NJJ=\t"+NJJ+"\t\t");}
			
			
//-------------------------------------------------------------------------------------------			
//			System.out.print("\nNEXTaResult\n");		
			File f2 = new File("D:/NEXTaResult/" + lf[ii].getName());
			long len = f2.length();
			if(len==0){
				System.out.print("file is empty!"+"\n\n");
			}
			else {
				Instances data = generateData("D:/NEXTaResult/"
						+ lf[ii].getName());
				InsNum = data.numInstances();

				st = System.currentTimeMillis();
				EM1.buildClusterer(data);
				...
				clusters = new int[m_num_clusters][InsNum + 1];
				for (int i = 0; i < InsNum; i++) {
					int cluIdex = EM1.clusterInstance(data.instance(i));
					clusters[cluIdex][0]++;
					clusters[cluIdex][clusters[cluIdex][0]] = i;
				}
				// System.out.print("\n");
				// for (int i =0;i<m_num_clusters;i++){
				// System.out.print(i+"
				// \t"+(double)clusters[i][0]/(double)InsNum+"\n");
				// }
				...
				if (m_num_clusters < 2)
					...
					for (int i = 0; i < m_num_clusters; i++) {
						for (int j = i + 1; j < m_num_clusters; j++) {
							VE = VE + clusters[i][0] * clusters[j][0];
							SumE = SumE
									+ TwoE(clusters[i], clusters[j], dataset,
											guiyi);
						}
					}
