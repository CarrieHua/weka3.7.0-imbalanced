...
public class Instances 
  ...
  */
  ...
  public void njj_deleteAttributeAt(int position) {
...
//	    }
  ...
  public Instances stratifiedResampleWithWeights(Random random,double[] weights){
	    if (weights.length != numInstances()) {
	        throw new IllegalArgumentException("weights.length != numInstances.");
	      }
	      Instances newData = new Instances(this, numInstances());
	      ...
	      //stratify the original data into minData and maxData
	      Instances minData= new Instances(this,0);
	      Instances maxData=new Instances(this,0);
	      for(int i=0;i<numInstances();i++){    	  
	    	  Instance ins=this.instance(i);
	    	  if(ins.classValue()==0){
	    		  minData.add(ins);
	      ...
	      //collect the weights for minData and maxData respectively
	      int minNumInstances=minData.numInstances();
	      int maxNumInstances=maxData.numInstances();
	      double[] minWeights= new double[minNumInstances];
	      double[] maxWeights= new double[maxNumInstances];
	      for(int i=0;i<minWeights.length;i++){
	    	  minWeights[i]=minData.instance(i).weight();
	      }
	      for(int j=0;j<maxWeights.length;j++){
	    	  maxWeights[j]=maxData.instance(j).weight();
	      }
	      
	      //sample with weights
	      Instances minSample=minData.resampleWithWeights(random,minWeights);
	      Instances maxSample=maxData.resampleWithWeights(random, maxWeights);
	      
	      Instances sampleData=new Instances(minSample);
	      maxSample.copyInstances(0, sampleData, maxNumInstances);
	     	      
	      sampleData.randomize(random);
	      return sampleData;
  }
