...
public class SMOTEclassifier
  ...
  //统计类别实例个数，默认第一个类为少数类，第二个类为多数类
    ...
    */
    ...
    if (nMin>1){
	    ...
	    filter.setInputFormat(data);  // filter capabilities are checked here
	    //data.
	    double value =100.0*nMaj/nMin -100;
	    //Percentage设置的是少数类增加的比例，计算让少数类和多数类一样多的值
	    filter.setPercentage(value);
	    //if (nMin<5) filter.setNearestNeighbors(nMin);
	    filter.setRandomSeed(nMaj*nMin+100);
	    //这里filter就是SMOTE了，SMOTE的参数要在
	    data = Filter.useFilter(data, filter);
	    //t.stop();
    }
  ...
  public static void main(String [] argv) {
    runClassifier(new SMOTEclassifier(), argv);
  }
}
